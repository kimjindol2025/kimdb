...
export class AIDatabase {
  ...
  async saveAIs(ais: SimpleAI[]): Promise<void> {
    ...
    // 트랜잭션으로 일괄 처리
    const transaction = this.db.transaction((aiList: SimpleAI[]) => {
      for (const ai of aiList) {
        insertStmt.run(
          ...
          ai.port,
          ai.personality,
          JSON.stringify(ai.skills),
          ai.status,
          ai.createdAt.toISOString(),
   ...
   * AI 상호작용 기록 조회
   */
  async getInteractions(aiId: string, limit: number = 10): Promise<AIInteraction[]> {
    ...
    const rows = stmt.all(aiId, limit) as any[];
  ...
  async createCollection(collection: {
    ...
    stmt.run(
      ...
      JSON.stringify(collection.aiIds),
      new Date().toISOString(),
  ...
  async getStats(): Promise<{
    ...
    const mostActiveStmt = this.db.prepare(`
      ...
      LIMIT 1
    `);
    const mostActive = mostActiveStmt.get() as any;
   ...
   * AI 검색 (스킬, 이름, 성격 기반)
   */
  async searchAIs(query: string, limit: number = 20): Promise<StoredAI[]> {
    const stmt = this.db.prepare(`
