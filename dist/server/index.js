...
import { CRDTDocument, UndoManager, PresenceManager, } from '../crdt/index.js';
const VERSION = '7.0.0';
// ===== LRU Cache =====
class LRUCache {
    maxSize;
...
export class KimDBServer {
    ...
    crdtDocs;
    presenceManagers = new Map();
    clientPresence = new Map();
    clientUndoManagers = new Map();
    // Metrics
    metrics = {
        startTime: Date.now(),
    ...
    getPresenceManager(collection, docId) {
        const key = `${collection}:${docId}`;
    ...
    getClientUndoManager(clientId, collection, docId) {
    ...
    localBroadcastToDoc(collection, docId, msgObj, excludeClientId) {
        const key = `${collection}:${docId}`;
    ...
    broadcastOp(collection, docId, operations, excludeClientId) {
        const msg = {
            type: 'crdt_sync',
            collection,
            docId,
    ...
    registerRoutes() {
        ...
        this.fastify.get('/api/c/:collection', async (req) => {
            const rows = this.db.getDocuments(req.params.collection);
            return {
                success: true,
            ...
            fastify.get('/ws', { websocket: true }, (socket) => {
                ...
                socket.on('message', (raw) => {
                    try {
                        ...
                        this.handleWebSocketMessage(clientId, socket, msg);
                    }
                    catch (e) {
    ...
    handleWebSocketMessage(clientId, socket, msg) {
        ...
        switch (msg.type) {
            case 'subscribe': {
                ...
                if (!this.subscriptions.has(col))
                    this.subscriptions.set(col, new Set());
            ...
            case 'subscribe_doc': {
                ...
                if (!this.docSubscriptions.has(key))
                    this.docSubscriptions.set(key, new Set());
            ...
            case 'crdt_get': {
                const doc = this.getCRDTDoc(msg.collection, msg.docId);
                send({
                    type: 'crdt_state',
            ...
            case 'crdt_set': {
                ...
                this.saveCRDTToDB(msg.collection, msg.docId, doc);
                this.broadcastOp(msg.collection, msg.docId, [op], clientId);
                const um = this.getClientUndoManager(clientId, msg.collection, msg.docId);
            ...
            case 'crdt_ops': {
                ...
                this.saveCRDTToDB(msg.collection, msg.docId, doc);
                this.broadcastOp(msg.collection, msg.docId, msg.operations, clientId);
    ...
    executeSQL(sql, params, collection) {
